...
const getCartContentById = async (req, res, next) => {
    ...
    try {
        ...
        const cartResult = await AppDataSource.query('SELECT user_id, cart_id, product_id, title, price, quantity FROM carts JOIN cart_items USING(cart_id) JOIN products USING(product_id) WHERE user_id = $1', [id]);
