...
const getOrderById = async (req, res, next) => {
    ...
    try {
        ...
        const orders = await AppDataSource.query('SELECT orders.order_id, orders.order_date, order_details.product_id, products.title, order_details.quantity FROM orders JOIN order_details USING(order_id) JOIN products USING(product_id) WHERE orders.user_id = $1 ORDER BY orders.order_id, order_details.product_id', [id]);
        if (orders.length > 0) {
...
const createOrder = async (req, res, next) => {
    ...
    try {
        ...
        const cartItems = await AppDataSource.query('SELECT c.product_id, c.quantity, p.price FROM cart_items c JOIN products p USING(product_id) WHERE c.cart_id = $1', [cart.cart_id]);
